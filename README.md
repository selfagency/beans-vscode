...
3. Create a feature branch: `git checkout -b feature/your-feature` (see `CONTRIBUTING.md` for naming conventions)
4. Make changes with tests (TDD-first: write failing tests first)
...
This section provides additional developer-focused guidance beyond basic contribution steps.

- TDD-first: write a failing test before implementing a feature. Tests live alongside implementation under `src/` and use Vitest.
- Branch naming: prefer `feature/<bean-id>-<short-slug>` or `feat/<ticket>-<short-slug>` so work can be linked to Beans tracking.
- Beans workflow: use Beans issues/epics for planning. When creating Beans epics, include a `## Todo` section and set status to `in-progress` when starting work.
- Copilot artifacts: Copilot instruction & skill files are generated by `src/beans/config/*` and written to `.github/instructions/` via `buildBeansCopilotInstructions` and `writeBeansCopilotInstructions`. Tests import the template markdown files directly using the vitest `.md` loader.
- MCP local run: after building (`pnpm run compile`), you can run the MCP server manually for local testing:

```bash
# Example: run MCP server locally on port 39173
node ./dist/beans-mcp-server.js --workspace . --cli-path beans --port 39173
```

If you encounter issues because your VS Code build lacks the `vscode.lm`/MCP APIs, use unit tests that mock the MCP APIs instead of relying on runtime registration.

See `CONTRIBUTING.md` for full contributor guidelines and the project TDD policy.
